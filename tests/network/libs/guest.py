...
def read_guest_interface_ipv4(
    ...
    output = vm_console_run_commands(vm=vm, commands=[cmd], timeout=30, command_output=True)
    if output is None:
        raise IpNotFound(f"Failed to run '{cmd}' on VM {vm.name}")

    cmd_output = output[cmd]
    LOGGER.info(f"Command {cmd} output: {cmd_output}")

    try:
        iface_info = json.loads(cmd_output[1])
    except (IndexError, json.JSONDecodeError) as err:
        raise IpNotFound(f"Failed to parse console JSON from VM {vm.name} for '{cmd}': {cmd_output}") from err
