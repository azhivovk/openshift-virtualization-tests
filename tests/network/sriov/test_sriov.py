...
@pytest.mark.parametrize(
    "ip_family",
    [
        pytest.param(4, marks=pytest.mark.ipv4),
        pytest.param(6, marks=pytest.mark.ipv6),
    ],
)
class TestPingConnectivity:
    ...
    def test_sriov_basic_connectivity(
        ...
        assert_ping_successful(
            src_vm=sriov_vm1,
            dst_ip=lookup_iface_status_ip(vm=sriov_vm2, iface_name=sriov_network.name, ip_family=ip_family),
        )
    ...
    def test_sriov_basic_connectivity_vlan(
        ...
        assert_ping_successful(
            src_vm=sriov_vm3,
            dst_ip=lookup_iface_status_ip(vm=sriov_vm4, iface_name=sriov_network_vlan.name, ip_family=ip_family),
        )
...
class TestSriovLiveMigration:
    ...
    def test_sriov_migration(
        ...
        assert_ping_successful(
            src_vm=sriov_vm2,
            dst_ip=lookup_iface_status_ip(vm=sriov_vm_migrate, iface_name=sriov_network.name, ip_family=ip_family),
        )
